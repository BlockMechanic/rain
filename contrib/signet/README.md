...
Syntax: `getcoins.py [-h|--help] [-c|--cmd=<rain-cli path>] [-f|--faucet=<faucet URL>] [-a|--addr=<signet bech32 address>] [-p|--password=<faucet password>] [--] [<rain-cli args>]`

* `--cmd` lets you customize the rain-cli path. By default it will look for it in the PATH
* `--faucet` lets you specify which faucet to use; the faucet is assumed to be compatible with https://github.com/kallewoof/rain-faucet
* `--addr` lets you specify a Signet address; by default, the address must be a bech32 address. This and `--cmd` above complement each other (i.e. you do not need `rain-cli` if you use `--addr`)
...
To mine the first block in your custom chain, you can run:
    ...
    cd src/
    CLI="./rain-cli -conf=mysignet.conf"
    MINER="..contrib/signet/miner"
    GRIND="./rain-util grind"
