#!/usr/bin/env bash
# Copyright (c) 2016 The Rain Core developers
# Distributed under the MIT software license, see the accompanying
# file COPYING or http://www.opensource.org/licenses/mit-license.php.

###   This script attempts to download the signature file SHA256SUMS.asc from
###   raincore.org and rain.org and compares them.
...
WORKINGDIR="/tmp/rain_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://raincore.org"
HOST2="https://rain.org"
BASEDIR="/bin/"
VERSIONPREFIX="rain-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'rain-',
...
if [ $? -ne 0 ]; then
   echo "rain.org failed to provide signature file, but raincore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "rain.org and raincore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Rain Core binary release signing key installed?"
   fi
