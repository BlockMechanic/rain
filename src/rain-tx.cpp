...
static void SetupRainTxArgs(ArgsManager &argsman)
{
...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs) || gArgs.IsArgSet("-version")) {
        ...
        std::string strUsage = PACKAGE_NAME " rain-tx utility version " + FormatFullVersion() + "\n";
        if (!gArgs.IsArgSet("-version")) {
            strUsage += "\n"
                "Usage:  rain-tx [options] <hex-tx> [commands]  Update hex-encoded rain transaction\n"
                "or:     rain-tx [options] -create [commands]   Create hex-encoded rain transaction\n"
                "\n";
...
static void MutateTxAddOutAddr(CMutableTransaction& tx, const std::string& strInput)
{
    ...
    CAsset asset = CAsset(uint256S(vStrInputParts[2]));
    if (asset.IsNull()) {
...
static void MutateTxAddOutPubKey(CMutableTransaction& tx, const std::string& strInput)
{
    ...
    // construct TxOut, append to transaction output list
    CTxOut txout(CAsset(), value, scriptPubKey);
    tx.vout.push_back(txout);
}
...
static void MutateTxAddOutData(CMutableTransaction& tx, const std::string& strInput)
{
    ...
    CTxOut txout(CAsset(), value, CScript() << OP_RETURN << data);
    tx.vout.push_back(txout);
}
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded rain transaction
