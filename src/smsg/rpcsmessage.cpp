        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    if (mode == "whitelist"
        ...
        for (auto it = smsgModule.addresses.begin(); it != smsgModule.addresses.end(); ++it) {
            ...
            WitnessV0KeyHash wpkh = WitnessV0KeyHash(keyID);
            ...
            objM.pushKV("address", EncodeDestination(WitnessV0KeyHash(keyID)));
        ...
        for (auto &p : smsgModule.keyStore.mapKeys) {
            ...
            objM.pushKV("address", EncodeDestination(WitnessV0KeyHash(p.first)));
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    const CKeyID &idk = ToKeyID(std::get<WitnessV0KeyHash>(dest));
    ...
    return RPCHelpMan{"smsgsend",
                "\nSend an encrypted message from \"address_from\" to \"address_to\".\n",
                {
                    ...
                    {"coin_control", RPCArg::Type::OBJ, /* default */ "", "",
                        {
                            {"changeaddress", RPCArg::Type::STR, /* default */ "", "The rain address to receive the change"},
...

    {
        ...
        if (mode == "all" || mode == "unread") {
            ...
            while (dbInbox.NextSmesg(it, smsg::DBK_INBOX, chKey, smsgStored)) {
                ...
                if (rv == 0) {
                    std::string sAddrTo = EncodeDestination(WitnessV0KeyHash(smsgStored.addrTo));
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    if (request.params.size() > 0) {
        ...
        if (!fMatchAll) {
            ...
            if (IsValidDestination(dest)) {
                CKeyID ki = ToKeyID(std::get<WitnessV0KeyHash>(dest));
        ...
        for (itp = vPrefixes.begin(); itp < vPrefixes.end(); ++itp) {
            ...
            while (dbMsg.NextSmesg(it, *itp, chKey, smsgStored)) {
                ...
                if ((rv = smsgModule.Decrypt(false, fInbox ? smsgStored.addrTo : smsgStored.addrOutbox,
                    ...
                    CKeyID kiFrom = ToKeyID(std::get<WitnessV0KeyHash>(dest));
                    ...
                    if ((itl = mLabelCache.find(kiFrom)) != mLabelCache.end()) {
                        ...
                        WitnessV0KeyHash wpkh = WitnessV0KeyHash(kiFrom);
                    ...
                    if ((itl = mLabelCache.find(smsgStored.addrTo)) != mLabelCache.end()) {
                        ...
                        WitnessV0KeyHash wpkh = WitnessV0KeyHash(smsgStored.addrTo);
                        lblTo = smsgModule.LookupLabel(wpkh);
                        mLabelCache[smsgStored.addrTo] = lblTo;
                    }

                    std::string sFrom = kiFrom.IsNull() ? "anon" : EncodeDestination(WitnessV0KeyHash(kiFrom));
                    std::string sTo = EncodeDestination(WitnessV0KeyHash(smsgStored.addrTo));
                    if (lblFrom.length() != 0) {
                        sFrom += " (" + lblFrom + ")";
                    }
                    if (lblTo.length() != 0) {
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    result.pushKV("to", EncodeDestination(WitnessV0KeyHash(smsgStored.addrTo)));
