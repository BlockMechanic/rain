...
public:
    ...
    std::string LookupLabel(WitnessV0KeyHash &hash);
    ...
    int ImportPrivkey(const std::string &vchSecret, const std::string &sLabel);
    ...
    int Send(CKeyID &addressFrom, CKeyID &addressTo, std::string &message,
        ...
        bool fTestFee=false, CAmount *nFee=nullptr, size_t *nTxBytes=nullptr, bool fFromFile=false, bool submit_msg=true, bool add_to_outbox=true, CCoinControl *coin_control=nullptr);
    ...
    int FundMsg(SecureMessage &smsg, std::string &sError, bool fTestFee, CAmount *nFee, size_t *nTxBytes, CCoinControl *coin_control);
