...
// Copyright (c) 202o-2021 The Rain Core developers
...

            {
                ...
                for (auto *pnode : smsg_module->m_node->connman->vNodes) {
                    ...
                    smsg_module->m_node->connman->PushMessage(pnode,
                        CNetMsgMaker(INIT_PROTO_VERSION).Make(NetMsgType::SMSGIGNORE, vchData));
...
int CSMSG::AddWalletAddresses()
{
    ...
    for (const auto &pw : m_vpwallets) {
        ...
        for (const auto &entry : pw->m_address_book) { // PAIRTYPE(CTxDestination, CAddressBookData)
            ...
            CKeyID keyID = ToKeyID(std::get<WitnessV0KeyHash>(entry.first));
    ...
    while (fgets(cLine, 512, fp))  {
        ...
        if (strcmp(pName, "key") == 0) {
            ...
            if (rv == 3) {
                ...
                CKeyID k = ToKeyID(std::get<WitnessV0KeyHash>(dest));
...
int CSMSG::WriteIni()
{
    ...
    for (std::vector<SecMsgAddress>::iterator it = addresses.begin(); it != addresses.end(); ++it) {
        ...
        CTxDestination dest(WitnessV0KeyHash(it->address));
        ...
        if (fprintf(fp, "key=%s|%d|%d\n", EncodeDestination(dest).c_str(), it->fReceiveEnabled, it->fReceiveAnon) < 0) {
            LogPrintf("fprintf error: %s\n", strerror(errno));
            continue;
        }
    }
    ...
    // Ping each peer advertising smsg
    {
        ...
        for (auto *pnode : m_node->connman->vNodes) {
            ...
            m_node->connman->PushMessage(pnode,
                CNetMsgMaker(INIT_PROTO_VERSION).Make(NetMsgType::SMSGPING)); // smsgData.fEnabled will be set on receiving smsgPong response from peer
            m_node->connman->PushMessage(pnode,
                CNetMsgMaker(INIT_PROTO_VERSION).Make(NetMsgType::SMSGPONG)); // Send pong as have missed initial ping sent by peer when it connected
        }
    }
...
bool CSMSG::Disable()
{
    ...
    // Tell each smsg enabled peer that this node is disabling
    {
        ...
        for (auto *pnode : m_node->connman->vNodes) {
            ...
            m_node->connman->PushMessage(pnode,
                CNetMsgMaker(INIT_PROTO_VERSION).Make(NetMsgType::SMSGDISABLED));
...
std::string CSMSG::LookupLabel(WitnessV0KeyHash &hash)
{
#ifdef ENABLE_WALLET
    for (const auto &pw : m_vpwallets) {
        LOCK(pw->cs_wallet);
...
int CSMSG::ReceiveData(PeerManager *peerLogic, CNode *pfrom, const std::string &strCommand, CDataStream &vRecv)
{
    ...
    */
    ...
    if (!fSecMsgEnabled) {
        if (strCommand == NetMsgType::SMSGPING) { // Ignore smsgPing
            return SMSG_NO_ERROR;
        }
    ...
    if (strCommand == NetMsgType::SMSGSHOW) {
        ...
        for (uint32_t i = 0; i < nBuckets; ++i, pIn += 8) {
            ...
            m_node->connman->PushMessage(pfrom,
                CNetMsgMaker(INIT_PROTO_VERSION).Make(NetMsgType::SMSGHAVE, vchDataOut));
        }
    } else
    if (strCommand == NetMsgType::SMSGHAVE) {
...

        {
            ...
            if (vchDataOut.size() > 8) {
                ...
                m_node->connman->PushMessage(pfrom,
                    CNetMsgMaker(INIT_PROTO_VERSION).Make(NetMsgType::SMSGWANT, vchDataOut));
            }
        } // cs_smsg
    } else
    if (strCommand == NetMsgType::SMSGWANT) {
        ...
        if (nBunch > 0) {
            ...
            m_node->connman->PushMessage(pfrom,
                CNetMsgMaker(INIT_PROTO_VERSION).Make(NetMsgType::SMSGMSG, vchBunch));
        }
    } else
    ...
    if (strCommand == NetMsgType::SMSGPING) {
        ...
        m_node->connman->PushMessage(pfrom,
            CNetMsgMaker(INIT_PROTO_VERSION).Make(NetMsgType::SMSGPONG, SMSG_VERSION));
    } else
    if (strCommand == NetMsgType::SMSGPONG) {
        ...
        if (!was_enabled) {
            //LOCK(pfrom->cs_vRecv);
            // Init counters
            //size_t num_types = ARRAYLEN(NetMsgType::SMSGallTypes);
            //for (size_t t = 0; t < num_types; ++t) {
            //    mapMsgCmdSize::iterator i = pfrom->mapRecvBytesPerMsgCmd.find(NetMsgType::SMSGallTypes[t]);
            //    if (i == pfrom->mapRecvBytesPerMsgCmd.end()) {
            //        pfrom->mapRecvBytesPerMsgCmd[NetMsgType::SMSGallTypes[t]] = 0;
            //    }
            //}
        }
    } else
...
bool CSMSG::SendData(CNode *pto, bool fSendTrickle)
{
    ...
    int64_t now = GetTime();
    {
        ...
        if (pto->smsgData.lastSeen <= 0) {
            ...
            m_node->connman->PushMessage(pto,
                CNetMsgMaker(INIT_PROTO_VERSION).Make(NetMsgType::SMSGPING));

            // Send smsgPong message if received smsgPing from peer while syncing chain
            if (pto->smsgData.lastSeen < 0) {
                m_node->connman->PushMessage(pto,
                    CNetMsgMaker(INIT_PROTO_VERSION).Make(NetMsgType::SMSGPONG));
            }
    ...
    if (nBucketsShown > 0) {
        ...
        m_node->connman->PushMessage(pto,
            CNetMsgMaker(INIT_PROTO_VERSION).Make(NetMsgType::SMSGINV, vchData));
    ...
    if (nBucketsContestReq > 0) {
        ...
        m_node->connman->PushMessage(pto,
            CNetMsgMaker(INIT_PROTO_VERSION).Make(NetMsgType::SMSGSHOW, vchData));
    }
    ...
    uint32_t &nTransactions, uint32_t &nElements, uint32_t &nPubkeys, uint32_t &nDuplicates) EXCLUSIVE_LOCKS_REQUIRED(cs_smsgDB)
{
    ...
    for (const auto &tx : block.vtx) {
        ...
        for (size_t i = 0; i < tx->vin.size(); i++) {
            ...
            const CScriptWitness &scriptWitness = tx->witness.vtxinwit.size() > i ? tx->witness.vtxinwit[i].scriptWitness : CScriptWitness();

            if (!scriptWitness.IsNull()) {
                std::vector<unsigned char> data(ParseHex(HexStr(scriptWitness.stack[1])));
                if (scriptWitness.stack.size() != 2) {
                    continue;
                }
                if (scriptWitness.stack[1].size() != 33) {
                    continue;
                }
                pubKey = CPubKey(scriptWitness.stack[1]);
            }
        ...
        switch(mode) {
            ...
            case CT_NEW:
                if (itFound == addresses.end()) {
                    ...
                    LogPrint(BCLog::SMSG, "%s: Already have address: %s.\n", __func__, EncodeDestination(WitnessV0KeyHash(keyId)));
...
int CSMSG::ScanMessage(const uint8_t *pHeader, const uint8_t *pPayload, uint32_t nPayload, bool reportToGui, bool &fOwnMessage, bool unlocking)
{
    ...
    for (auto &p : smsgModule.keyStore.mapKeys) {
        ...
        if (!(key.nFlags & SMK_RECEIVE_ANON)) {
            ...
            if (Decrypt(false, key.key, address, pHeader, pPayload, nPayload, msg) == 0) {
                if (LogAcceptCategory(BCLog::SMSG)) {
                    LogPrintf("Decrypted message with %s.\n", EncodeDestination(WitnessV0KeyHash(addressTo)));
                }
    ...
    if (!fOwnMessage) {
        ...
        for (std::vector<SecMsgAddress>::iterator it = addresses.begin(); it != addresses.end(); ++it) {
            ...
            for (const auto &pw : m_vpwallets) {
                ...
                if (pw->IsLocked()) {
                    if (spk_man->HaveKey(addressTo)) {
                        was_locked = true;
                    }
                    continue;
                }
                if (spk_man->GetKey(addressTo, keyDest)) {
                    break;
                }
            }
            if (!keyDest.IsValid()) {
    ...
    if (fOwnMessage) {
        ...
        if (!fExisted) {
            ...
            //TODO: Format message
            if (!strCmd.empty()) {
                boost::replace_all(strCmd, "%s", HexStr(Span<const unsigned char>(&chKey[2], 28)));
...
int CSMSG::GetLocalKey(const CKeyID &ckid, CPubKey &cpkOut)
{
    ...
    for (const auto &pw : m_vpwallets) {
        ...
        if (spk_man->GetPubKey(ckid, cpkOut) && cpkOut.IsValid()) {
            return SMSG_NO_ERROR;
        }
    }
#endif
...
int CSMSG::GetLocalKey(const CKeyID &key_id, CKey &key_out)
{
    ...
    for (const auto &pw : m_vpwallets) {
        ...
        if (spk_man->GetKey(key_id, key_out)) {
            return SMSG_NO_ERROR;
        }
    }
#endif
...
int CSMSG::GetLocalPublicKey(const std::string &strAddress, std::string &strPublicKey)
{
    ...
    CKeyID keyID = ToKeyID(std::get<WitnessV0KeyHash>(dest));
...
int CSMSG::AddAddress(std::string &address, std::string &publicKey)
{
    ...
    CKeyID idk = ToKeyID(std::get<WitnessV0KeyHash>(dest));
...
int CSMSG::ImportPrivkey(const std::string &strSecret, const std::string &sLabel)
{
    SecMsgKey key;
    key.key = DecodeSecret(strSecret);
    key.sLabel = sLabel;
...
int CSMSG::StoreFundingTx(const CTransaction &tx, const CBlockIndex *pindex)
{
	...
	if (!tx.data.IsType(DataTypes::OUTPUT_DATA)) {
        return errorN(SMSG_GENERAL_ERROR, "%s Tx: %s is not output data.\n", __func__, tx.GetHash().ToString());
	}
	const std::vector<uint8_t> &output_data = *tx.data.GetPData();
	if (output_data.size() < 25 || output_data[0] != DO_FUND_MSG) {
        return errorN(SMSG_GENERAL_ERROR, "%s Tx: %s has bad output data.\n", __func__, tx.GetHash().ToString());
...
int CSMSG::Encrypt(SecureMessage &smsg, const CKeyID &addressFrom, const CKeyID &addressTo, const std::string &message)
{
    ...
    if (LogAcceptCategory(BCLog::SMSG)) {
        LogPrint(BCLog::SMSG, "SecureMsgEncrypt(%s, %s, ...)\n",
            fSendAnonymous ? "anon" : EncodeDestination(WitnessV0KeyHash(addressFrom)),
            EncodeDestination(WitnessV0KeyHash(addressTo)));
    }
    ...
    if (!IsValidDestination(WitnessV0KeyHash(addressFrom))) {
        return errorN(SMSG_INVALID_ADDRESS, "%s - addressFrom is not valid: %s.", __func__, EncodeDestination(WitnessV0KeyHash(addressFrom)));
    }
    CKeyID ckidFrom = addressFrom;
    CKey keyFrom;

    if (!fSendAnonymous) {
        if (addressFrom.IsNull())
            return errorN(SMSG_INVALID_ADDRESS, "%s - addressFrom is not valid failed: %s.", __func__, EncodeDestination(WitnessV0KeyHash(addressFrom)));
    }
    ...
    if (fSendAnonymous) {
        ...
        vchPayload[0] = sourcedata[0]; // vchPayload[0] = coinAddrDest.nVersion;
...
int CSMSG::Send(CKeyID &addressFrom, CKeyID &addressTo, std::string &message,
    ...
    size_t nRetention, bool fTestFee, CAmount *nFee, size_t *nTxBytes, bool fFromFile, bool submit_msg, bool add_to_outbox, CCoinControl *coin_control)
{
    bool fSendAnonymous = (addressFrom.IsNull());

    if (LogAcceptCategory(BCLog::SMSG)) {
        LogPrintf("SecureMsgSend(%s, %s, ...)\n",
            fSendAnonymous ? "anon" : EncodeDestination(WitnessV0KeyHash(addressFrom)), EncodeDestination(WitnessV0KeyHash(addressTo)));
    }
    ...
    if (fPaid) {
        if (0 != FundMsg(smsg, sError, fTestFee, nFee, nTxBytes, coin_control)) {
    ...
    if (!pactive_wallet) {
        ...
        for (const auto &entry : pactive_wallet->m_address_book) { // PAIRTYPE(CTxDestination, CAddressBookData)
            ...
            TxoutType whichType = Solver(GetScriptForDestination(entry.first), vSolutions);

            if (whichType == TxoutType::WITNESS_V0_KEYHASH){
                addressOutbox = ToKeyID(std::get<WitnessV0KeyHash>(entry.first));
                break;
            }
        }
    }
...
int CSMSG::FundMsg(SecureMessage &smsg, std::string &sError, bool fTestFee, CAmount *nFee, size_t *nTxBytes, CCoinControl *coin_control)
{
    ...
    CAmountMap nFeeRet;
    {
        ...
        CWallet *const pw = pactive_wallet.get();
        CMutableTransaction txNew;
        bilingual_str error;
        std::set<int> setSubtractFeeFromOutputs;
        int nChangePosInOut = -1;

        if (! pw->FundTransaction(txNew, nFeeRet, nChangePosInOut, error, true, setSubtractFeeFromOutputs, *coin_control)) {
            return SMSG_FUND_FAILED;
        }

        if (nFee) {
            *nFee = nFeeRet.begin()->second;
        }
        if (nTxBytes) {
            *nTxBytes = GetVirtualTransactionSize(CTransaction(txNew));
        }
        ...
        CWalletTx wtx(pactive_wallet.get(), MakeTransactionRef(txNew));
...
{
    if (LogAcceptCategory(BCLog::SMSG)) {
        LogPrintf("%s: using %s, testonly %d.\n", __func__, EncodeDestination(WitnessV0KeyHash(address)), fTestOnly);
    }
    ...
    if (fFromAnonymous) {
        ...
        if (!IsValidDestination(CTxDestination(WitnessV0KeyHash(ckidFrom)))) {
        ...
        CTxDestination destr(WitnessV0KeyHash(cpkFromSig.GetID()));

        if (!IsValidDestination(destr)) {
            return errorN(SMSG_GENERAL_ERROR, "%s: Signature validation failed.", __func__);
        }
        ...
        msg.sFromAddress = EncodeDestination(WitnessV0KeyHash(ckidFrom));
    }

    if (LogAcceptCategory(BCLog::SMSG)) {
        LogPrintf("Decrypted message for %s.\n", EncodeDestination(WitnessV0KeyHash(address)));
    }
...
int CSMSG::Decrypt(bool fTestOnly, const CKeyID &address, const uint8_t *pHeader, const uint8_t *pPayload, uint32_t nPayload, MessageData &msg)
{
    ...
    if (!keyDest.IsValid()) {
        for (const auto &pw : smsgModule.m_vpwallets) {
            ...
            if (pw->IsLocked()) {
                if (spk_man->HaveKey(address)) {
                    return SMSG_WALLET_LOCKED;
                }
                continue;
            }
            spk_man->GetKey(address, keyDest);
            if (keyDest.IsValid()) {
                break;
            }
        }
