...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "raind -foo=bar
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //rain-tx using stdin
        std::string val;
...
fs::path GetDefaultDataDir()
{
    ...
    // Unix: ~/.rain
    // Android ~/mnt/sdcard/.rain

    //FILE sdCard = Environment.getExternalStorageDirectory();
    //FILE dir = new FILE (sdCard.getAbsolutePath() + "/.rain");
    //dir.mkdirs();
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Rain";
#else
    // Unix
    return pathRet / ".rain";
#endif
...
{
    std::string strCopyrightHolders = "Rain Developers";//strPrefix + strprintf(_(COPYRIGHT_HOLDERS), _(COPYRIGHT_HOLDERS_SUBSTITUTION));

    // Check for untranslated substitution to make sure Rain Core copyright is not removed by accident
   // if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION).find("Rain Core") == std::string::npos) {
  //      strCopyrightHolders += "\n" + strPrefix + "The Rain Core developers";
   // }
