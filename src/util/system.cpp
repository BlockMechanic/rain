...
const char * const RAIN_SETTINGS_FILENAME = "settings.json";
...
bool ArgsManager::GetSettingsPath(fs::path* filepath, bool temp) const
{
    ...
    if (filepath) {
        std::string settings = GetArg("-settings", RAIN_SETTINGS_FILENAME);
...
{
    // Windows: C:\Users\Username\AppData\Roaming\Rain
    // macOS: ~/Library/Application Support/Rain
    // Unix-like: ~/.rain
#ifdef WIN32
    // Windows
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    // Make sure Rain Core copyright is not removed by accident
    if (copyright_devs.find("Rain Core") == std::string::npos) {
