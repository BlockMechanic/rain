...
std::string GetHelpString(int nParamNum, std::string strParamName)
{
    static const std::map<std::string, std::string> mapParamHelp = {
        {"collateralAddress",
            "%d. \"collateralAddress\"        (string, required) The rain address to send the collateral to.\n"
        },
        {"collateralHash",
        ...
        {"ownerAddress",
            "%d. \"ownerAddress\"             (string, required) The rain address to use for payee updates and proposal voting.\n"
        ...
        {"payoutAddress_register",
            "%d. \"payoutAddress\"            (string, required) The rain address to use for masternode reward payments.\n"
        },
...
// Allows to specify Rain address or priv key. In case of Rain address, the priv key is taken from the wallet
...
static CKeyID ParsePubKeyIDFromAddress(const std::string& strAddress, const std::string& paramName)
{
	...
	if (!IsValidDestination(dest) || keyID.IsNull()) {
...
static void FundSpecialTx(CWallet* pwallet, CMutableTransaction& tx, const SpecialTxPayload& payload, const CTxDestination& fundDest, CAsset asset)
{
    ...
    pwallet->AvailableCoins(*locked_chain,vecOutputs, true, &coinControl);
...
void protx_register_fund_help(CWallet* const pwallet)
{
    throw std::runtime_error(
            ...
            "\nCreates, funds and sends a ProTx to the network. The resulting transaction will move 1000 Rain\n"
...
UniValue protx_register(const JSONRPCRequest& request)
{
    ...
    CKey keyOwner = ParsePrivKey(pwallet.get(), request.params[paramIdx + 1].get_str(), true);
...
UniValue protx_register_submit(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 3) {
        protx_register_submit_help(pwallet.get());
    }
...
UniValue protx_update_service(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || (request.params.size() < 4 || request.params.size() > 6))
        protx_update_service_help(pwallet.get());
...
UniValue protx_update_registrar(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || (request.params.size() != 5 && request.params.size() != 6)) {
        protx_update_registrar_help(pwallet.get());
    }
...
static bool CheckWalletOwnsScript(CWallet* pwallet, const CScript& script) {
    ...
    if (ExtractDestination(script, dest)) {
		CKeyID keyId = CKeyID(*boost::get<PKHash>(&dest));
		if (keyId.IsNull()) {
			throw JSONRPCError(RPC_INVALID_PARAMETER, "must be a valid address");
		}
        if ((boost::get<PKHash>(&dest) && pwallet->HaveKey(keyId)) || (boost::get<ScriptHash>(&dest) && pwallet->HaveCScript(*boost::get<ScriptHash>(&dest)))) {
            return true;
        }
    }
    return false;
    ...
    if (type == "wallet") {
        ...
        mnList.ForEachMN(false, [&](const CDeterministicMNCPtr& dmn) {
            if (setOutpts.count(dmn->collateralOutpoint) ||
                CheckWalletOwnsKey(pwallet.get(), dmn->pdmnState->keyIDOwner) ||
                CheckWalletOwnsKey(pwallet.get(), dmn->pdmnState->keyIDVoting) ||
                CheckWalletOwnsScript(pwallet.get(), dmn->pdmnState->scriptPayout) ||
                CheckWalletOwnsScript(pwallet.get(), dmn->pdmnState->scriptOperatorPayout)) {
                ret.push_back(BuildDMNListEntry(pwallet.get(), dmn, detailed));
            }
        });
#endif
...
UniValue protx_info(const JSONRPCRequest& request)
{
    ...
    return BuildDMNListEntry(pwallet.get(), dmn, true);
}
