...
UniValue blockheaderToJSON(const CBlockIndex* tip, const CBlockIndex* blockindex)
{
	...
	for(auto elem : pindexBestHeader->nMoneySupply){
		supplyobj.pushKV(elem.first.getName(),  ValueFromAmount(elem.second));	
	}
                        ...
                        {RPCResult::Type::OBJ, "scriptPubKey", "",
                            {
                                ...
                                {RPCResult::Type::ARR, "addresses", "array of rain addresses",
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    if (block.IsProofOfStake()) {
        ...
        if (!GetCoinAge(*block.vtx[1], coins_view, nCoinAge))
            return error("ConnectBlock() : %s unable to get coin age for coinstake", block.vtx[1]->GetHash().ToString());
    }
    CAmountMap reward = GetBlockSubsidy(pindex->nHeight, Params().GetConsensus(), pindex->IsProofOfStake(), nCoinAge, pindex->nMoneySupply);
    std::string rewards = mapToString(reward);
    ...
    ret_all.pushKV("subsidy", rewards);
...
UniValue CreateUTXOSnapshot(NodeContext& node, CChainState& chainstate, CAutoFile& afile)
{
...

    {
        ...
        //
        // See discussion here:
        //   https://github.com/rain/rain/pull/15606#discussion_r274479369
        //
        LOCK(::cs_main);
