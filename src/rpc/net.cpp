...
static UniValue getpeerinfo(const JSONRPCRequest& request)
{
    ...
    for (const CNodeStats& stats : vstats) {
        ...
        obj.pushKV("minfeefilter", ValueFromAmountMap(stats.minFeeFilter));
...
static UniValue getaddednodeinfo(const JSONRPCRequest& request)
{
            ...
            "       {\n"
            "         \"address\" : \"192.168.0.201:8333\",  (string) The rain server IP and port we're connected to\n"
...
static UniValue getnetworkinfo(const JSONRPCRequest& request)
{
    ...
    obj.pushKV("relayfee",      ValueFromAmountMap(::minRelayTxFee.GetFeePerK()));
    obj.pushKV("incrementalfee", ValueFromAmountMap(::incrementalRelayFee.GetFeePerK()));
