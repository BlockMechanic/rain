...
UniValue gobject_prepare(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || (request.params.size() != 5 && request.params.size() != 6 && request.params.size() != 8)) 
        gobject_prepare_help(pwallet.get());
    ...
    // This command is dangerous because it consumes 5 RAIN irreversibly.
    // If params are lost, it's very hard to bruteforce them and yet
    // users ignore all instructions on raincentral etc. and do not save them...
    ...
    mapValue_t mapValue;
    if (!pwallet->CommitTransaction(wtx.tx, std::move(mapValue), {}, state)) {
...
void gobject_vote_conf_help()
{
    throw std::runtime_error(
                ...
                "Vote on a governance object by masternode configured in rain.conf\n"
                "\nArguments:\n"
...
UniValue gobject_vote_conf(const JSONRPCRequest& request)
{
    ...
    if (!dmn) {
        ...
        resultsObj.pushKV("rain.conf", statusObj);
...
UniValue gobject_vote_many(const JSONRPCRequest& request)
{
    std::shared_ptr<CWallet> const pwallet = GetWalletForJSONRPCRequest(request);
    if (request.fHelp || request.params.size() != 4)
        gobject_vote_many_help(pwallet.get());
...
UniValue gobject_vote_alias(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 5)
        gobject_vote_alias_help(pwallet.get());
...
#endif // ENABLE_WALLET
            "  vote-conf          - Vote on a governance object by masternode configured in rain.conf\n"
#ifdef ENABLE_WALLET
...
UniValue getsuperblockbudget(const JSONRPCRequest& request)
{
    ...
    return ValueFromAmountMap(CSuperblock::GetPaymentsLimit(nBlockHeight));
}
...
{ //  category              name                      actor (function)         okSafe argNames
  //  --------------------- ------------------------  -----------------------  ------ ----------
    /* Rain features */
    { "rain",               "getgovernanceinfo",      &getgovernanceinfo,        {} },
    { "rain",               "getsuperblockbudget",    &getsuperblockbudget,      {"index"} },
    { "rain",               "gobject",                &gobject,                  {} },
    { "rain",               "voteraw",                &voteraw,                  {} },
