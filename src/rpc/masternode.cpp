...
void masternode_list_help()
{
    throw std::runtime_error(
            ...
            "  owneraddress   - Print the masternode owner Rain address\n"
            "  payee          - Print the masternode payout Rain address (can be additionally filtered,\n"
            "                   partial match)\n"
            "  pubKeyOperator - Print the masternode operator public key\n"
            "  status         - Print masternode status: ENABLED / POSE_BANNED\n"
            "                   (can be additionally filtered, partial match)\n"
            "  votingaddress  - Print the masternode voting Rain address\n"
        );
}
...
UniValue masternode_connect(const JSONRPCRequest& request)
{
    ...
    if (!Lookup(strAddress.c_str(), addr, 0, false))
        throw JSONRPCError(RPC_MISC_ERROR, strprintf("Incorrect masternode address %s", strAddress));
    ...
    if (!g_connman->IsConnected(CAddress(addr, NODE_NETWORK), CConnman::AllNodes))
        throw JSONRPCError(RPC_MISC_ERROR, strprintf("Couldn't connect to masternode %s", strAddress));
