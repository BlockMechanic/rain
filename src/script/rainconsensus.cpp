...
inline int set_error(rainconsensus_error* ret, rainconsensus_error serror)
{
    if (ret)
        *ret = serror;
    return 0;
                                    ...
                                    unsigned int nIn, unsigned int flags, rainconsensus_error* err)
{
    ...
    try {
        ...
        if (GetSerializeSize(tx, PROTOCOL_VERSION) != txToLen)
            return set_error(err, rainconsensus_ERR_TX_SIZE_MISMATCH);
...
int rainconsensus_verify_script(const unsigned char *scriptPubKey, unsigned int scriptPubKeyLen,
                                   const unsigned char *txTo        , unsigned int txToLen,
                                   unsigned int nIn, unsigned int flags, rainconsensus_error* err)
{
