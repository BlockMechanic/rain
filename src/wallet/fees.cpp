...
CFeeRate GetMinimumFeeRate(const CWallet& wallet, const CCoinControl& coin_control, FeeCalculation* feeCalc)
{
    ...
    */
    ...
    if (coin_control.m_feerate) { // 1.
    ...
    else if (!coin_control.m_confirm_target && wallet.m_pay_tx_fee != CFeeRate(populateMap(0))) { // 3. TODO: remove magic value of 0 for wallet member m_pay_tx_fee
        ...
        if (feerate_needed == CFeeRate(populateMap(0))) {
            // if we don't have enough data for estimateSmartFee, then use fallback fee
            feerate_needed = wallet.m_fallback_fee;
            if (feeCalc) feeCalc->reason = FeeReason::FALLBACK;

            // directly return if fallback fee is disabled (feerate 0 == disabled)
            if (wallet.m_fallback_fee == CFeeRate(populateMap(0))) return feerate_needed;
        }
...
CFeeRate GetDiscardRate(const CWallet& wallet)
{
    ...
    discard_rate = (discard_rate == CFeeRate(populateMap(0))) ? wallet.m_discard_rate : std::min(discard_rate, wallet.m_discard_rate);
