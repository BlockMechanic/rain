...
BOOST_AUTO_TEST_CASE(blockfilter_basic_test)
{
    ...
    block_undo.vtxundo.back().vprevout.emplace_back(CTxOut(500, included_scripts[3]), 1000, true, false, block.nTime);
    block_undo.vtxundo.back().vprevout.emplace_back(CTxOut(600, included_scripts[4]), 10000, false, false, block.nTime);
    block_undo.vtxundo.back().vprevout.emplace_back(CTxOut(700, excluded_scripts[3]), 100000, false, false, block.nTime);

    BlockFilter block_filter(BlockFilterType::BASICS, block, block_undo);
...
BOOST_AUTO_TEST_CASE(blockfilters_json_test)
{
    ...
    for (unsigned int i = 0; i < tests.size(); i++) {
        ...
        for (unsigned int ii = 0; ii < prev_scripts.size(); ii++) {
            ...
            tx_undo.vprevout.emplace_back(txout, 0, false,false, block.nTime);
        }
        ...
        BlockFilter computed_filter_basic(BlockFilterType::BASICS, block, block_undo);
...
{
    BOOST_CHECK_EQUAL(BlockFilterTypeName(BlockFilterType::BASICS), "basic");
    ...
    BOOST_CHECK_EQUAL(filter_type, BlockFilterType::BASICS);
