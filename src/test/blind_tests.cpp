// Copyright (c) 2013-2020 The Elements Core developers
...
BOOST_AUTO_TEST_CASE(naive_blinding_test)
{
    ...
    CTxOut btc_oo(rainID, 11, CScript());
    CTxOut btc_ooo(rainID, 111, CScript());
...

    {
        ...
        tx3.vout.resize(0);
        tx3.vout.push_back(CTxOut(rainID, 100, CScript() << OP_TRUE));
        // Fee outputs are blank scriptpubkeys, and unblinded value/asset
        tx3.vout.push_back(CTxOut(rainID, 22, CScript()));
        ...
        tx3.vout.push_back(CTxOut(rainID, 0, CScript() << OP_RETURN));
        ...
        BOOST_CHECK(unblinded_id == rainID);
        CAsset temp_asset;
...

    {
        ...
        tx4.vout.push_back(CTxOut(rainID, 30, CScript() << OP_TRUE));
        tx4.vout.push_back(CTxOut(rainID, 40, CScript() << OP_TRUE));
        tx4.vout.push_back(CTxOut(rainID, 111+100-30-40, CScript()));
...

    {
        ...
        tx4.vout.push_back(CTxOut(rainID, 30, CScript() << OP_TRUE));
        tx4.vout.push_back(CTxOut(rainID, 40, CScript() << OP_TRUE));
        tx4.vout.push_back(CTxOut(rainID, 50, CScript() << OP_TRUE));
        // Fee
        tx4.vout.push_back(CTxOut(rainID, 111+100-30-40-50, CScript()));
        BOOST_CHECK(!VerifyAmounts(inputs, CTransaction(tx4), nullptr, false)); // Spends a blinded coin with no blinded outputs to compensate.
    ...
    }
    {
        ...
        // Spends 100 blinded rain, 500 of unblinded "other"
        ...
        tx5.vout.push_back(CTxOut(rainID, 29, CScript() << OP_TRUE));
        tx5.vout.push_back(CTxOut(rainID, 70, CScript() << OP_TRUE));
        tx5.vout.push_back(CTxOut(otherID, 250, CScript() << OP_TRUE));
        tx5.vout.push_back(CTxOut(otherID, 249, CScript() << OP_TRUE));
        // Fees
        tx5.vout.push_back(CTxOut(rainID, 1, CScript()));
