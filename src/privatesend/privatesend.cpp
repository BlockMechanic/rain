...
bool CPrivateSendBroadcastTx::IsValidStructure()
{
    ...
    for (const auto& out : tx->vout) {
        ...
        if (!out.scriptPubKey.IsPayToPubkeyHash()) {
            return false;
        }
    }
    return true;
...
bool CPrivateSend::IsCollateralValid(const CTransactionRef& txCollateral)
{
    ...
    for (const auto& txout : txCollateral->vout) {
        ...
        if (!txout.scriptPubKey.IsPayToPubkeyHash() && !txout.scriptPubKey.IsUnspendable()) {
...

    {
        ...
        if (!AcceptToMemoryPool(mempool, validationState, txCollateral, &fMissingInputs2, nullptr, false, 0)) {
