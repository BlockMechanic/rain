...
void CPrivateSendServer::ProcessMessage(CNode* pfrom, const std::string& strCommand, CDataStream& vRecv, CConnman& connman)
{
    ...
    if (fLiteMode) return; // ignore all Rain related functionality
...
void CPrivateSendServer::CommitFinalTransaction(CConnman& connman)
{
...

    {
        ...
        if (!lockMain || !AcceptToMemoryPool(mempool, validationState, finalTransaction, &fMissingInputs2, nullptr, false, 0)) {
...
/*
    ...
    allow endless transaction that would bloat Rain and make it unusable. To
...
void CPrivateSendServer::ConsumeCollateral(CConnman& connman, const CTransactionRef& txref)
{
    ...
    CValidationState validationState;
    bool fMissingInputs2 = false;
    if (!AcceptToMemoryPool(mempool, validationState, txref, &fMissingInputs2, nullptr, false, 0)) {
...
bool CPrivateSendServer::IsInputScriptSigValid(const CTxIn& txin)
{
    ...
    if (nTxInIndex >= 0) { //might have to do this one input at a time?
        ...
        if (!VerifyScript(txNew.vin[nTxInIndex].scriptSig, sigPubKey, &(txNew.vin[nTxInIndex].scriptWitness),SCRIPT_VERIFY_P2SH | SCRIPT_VERIFY_STRICTENC, MutableTransactionSignatureChecker(&txNew, nTxInIndex, 0), nullptr)) {
...
bool CPrivateSendServer::IsAcceptableDSA(const CPrivateSendAccept& dsa, PoolMessage& nMessageIDRet)
{
    ...
    // check collateral
    if (!fUnitTest && !CPrivateSend::IsCollateralValid(MakeTransactionRef(CTransaction(dsa.txCollateral)))) {
...
{
    if (fLiteMode) return;        // disable all Rain specific functionality
