...
std::string CPrivateSendClientSession::GetStatus(bool fWaitForBlock)
{
    ...
    case POOL_STATE_QUEUE:
        if (nStatusMessageProgress % 70 <= 30)
            ...
            strSuffix = "...";
        return strprintf("Submitted to masternode, waiting in queue %s", strSuffix);
    ...
    case POOL_STATE_SIGNING:
        if (nStatusMessageProgress % 70 <= 40)
            ...
            strSuffix = "...";
        return strprintf("Found enough users, signing ( waiting %s )", strSuffix);
    ...
    default:
        return strprintf("Unknown state: id = %u", nState);
    }
}
...
bool CPrivateSendClientSession::CheckPoolStateUpdate(CPrivateSendStatusUpdate psssup)
{
    ...
    strAutoDenomResult = "Masternode: " + CPrivateSend::GetMessageByID(psssup.nMessageID);
...
bool CPrivateSendClientSession::SignFinalTransaction(const CTransaction& finalTransactionNew, CNode* pnode, CConnman& connman)
{
    ...
    CMutableTransaction rtx(finalTransactionNew);

    finalMutableTransaction = rtx;
    ...
    for (const auto& entry : vecEntries) {
        ...
        for (const auto& txdsin : entry.vecTxDSIn) {
            ...
            // TODO we're using amount=0 here but we should use the correct amount. This works because Rain ignores the amount while signing/verifying (only used in Rain/Segwit)
            if (!SignSignature(*GetWallets()[0], prevPubKey, finalMutableTransaction, nMyInputIndex, 0, int(SIGHASH_ALL | SIGHASH_ANYONECANPAY))) { // changes scriptSig
...
bool CPrivateSendClientManager::CheckAutomaticBackup()
{
    ...
    if (GetWallets()[0]->nKeysLeftSinceAutoBackup < PRIVATESEND_KEYS_THRESHOLD_STOP) {
        ...
        strAutoDenomResult = strprintf("Very low number of keys left: %d no mixing available.", GetWallets()[0]->nKeysLeftSinceAutoBackup);
        ...
        strAutoDenomResult = strprintf("Very low number of keys left: %d", GetWallets()[0]->nKeysLeftSinceAutoBackup);
        ...
        if (fCreateAutoBackups) {
            ...
            if (!AutoBackupWallet((GetWallets()[0]).get(), "", warningString, errorString)) {
                ...
                if (!errorString.empty()) {
                    ...
                    strAutoDenomResult = strprintf("ERROR! Failed to create automatic backup : %s", errorString);
                    return false;
                }
            }
        } else {
...
bool CPrivateSendClientSession::DoAutomaticDenominating(CConnman& connman, bool fDryRun)
{
...

    {
        ...
        if (!fDryRun && GetWallets()[0]->IsLocked()) {
        ...
        if (txMyCollateral == CMutableTransaction()) {
            ...
            if (!CPrivateSend::IsCollateralValid(MakeTransactionRef(CTransaction(txMyCollateral)))) {
...
bool CPrivateSendClientSession::JoinExistingQueue(CAmount nBalanceNeedsAnonymized, CConnman& connman)
{
    ...
    while (privateSendClient.GetQueueItemAndTry(dsq)) {
        ...
        if (!GetWallets()[0]->SelectPSInOutPairsByDenominations(dsq.nDenom, nMinAmount, nMaxAmount, vecPSInOutPairsTmp)) {
...
bool CPrivateSendClientSession::StartNewQueue(CAmount nBalanceNeedsAnonymized, CConnman& connman)
{
    ...
    if (!GetWallets()[0]->SelectPrivateCoins(CPrivateSend::GetSmallestDenomination(), nBalanceNeedsAnonymized, vecTxIn, nValueInTmp, 0, privateSendClient.nPrivateSendRounds - 1)) {
    ...
    while (nTries < 10) {
        ...
        GetWallets()[0]->ConvertList(vecTxIn, vecAmounts);
...
bool CPrivateSendClientSession::SelectDenominate(std::string& strErrorRet, std::vector<std::pair<CTxDSIn, CTxOut> >& vecPSInOutPairsRet)
{
    ...
    bool fSelected = GetWallets()[0]->SelectPSInOutPairsByDenominations(nSessionDenom, vecStandardDenoms[vecBits.front()], CPrivateSend::GetMaxPoolAmount(), vecPSInOutPairsRet);
    if (!fSelected) {
        ...
        for (const auto& nBit : vecBits) {
            ...
            if (pair.second.nValue == nValueDenom) {
                ...
                if (fDryRun) {
                    ...
                    scriptDenom = keyHolderStorage->AddKey(GetWallets()[0].get());
                }
...
bool CPrivateSendClientSession::MakeCollateralAmounts(const CompactTallyItem& tallyItem, bool fTryDenominated, CConnman& connman)
{
	...
	ReserveDestination reservedest(GetWallets()[0].get());
	CTxDestination dest;
	bool ret = reservedest.GetReservedDestination(GetWallets()[0]->m_default_change_type, dest, true);
    
    CScript scriptCollateral = GetScriptForDestination(dest);
    ...
    auto locked_chain = GetWallets()[0]->chain().lock();

    bool fSuccess = GetWallets()[0]->CreateTransaction(*locked_chain, vecSend, tx, nFeeRet, nChangePosRet, strFail, coinControl);
    if (!fSuccess) {
        ...
        if (fTryDenominated) {
            ...
            if (!GetWallets()[0]->CreateTransaction(*locked_chain, vecSend, tx,  nFeeRet, nChangePosRet, strFail, coinControl)) {
    ...
    mapValue_t mapValue;
    if (!GetWallets()[0]->CommitTransaction(tx, std::move(mapValue), {}, state)) {
...
bool CPrivateSendClientSession::CreateDenominated(CAmount nBalanceToDenominate, const CompactTallyItem& tallyItem, bool fCreateMixingCollaterals, CConnman& connman)
{
    ...
    if (fCreateMixingCollaterals) {
        CScript scriptCollateral = keyHolderStorageDenom.AddKey(GetWallets()[0].get());
        vecSend.push_back((CRecipient){scriptCollateral, CPrivateSend::GetMaxCollateralAmount(), false});
    ...
    for (auto it = vecStandardDenoms.rbegin(); it != vecStandardDenoms.rend(); ++it) {
        ...
        if (privateSendClient.IsDenomSkipped(nDenomValue)) {
            strAutoDenomResult = strprintf("Too many %f denominations, skipping.", (float)nDenomValue / COIN);
        ...
        if (nDenomValue != vecStandardDenoms.front() && GetWallets()[0]->CountInputsWithAmount(nDenomValue) > privateSendClient.nPrivateSendDenoms) {
            strAutoDenomResult = strprintf("Too many %f denominations, removing.", (float)nDenomValue / COIN);
            LogPrint(BCLog::PRIVATESEND, "CPrivateSendClientSession::CreateDenominated -- %s\n", strAutoDenomResult);
        ...
        while (needMoreOutputs() && nOutputs <= 10) {
            CScript scriptDenom = keyHolderStorageDenom.AddKey(GetWallets()[0].get());
...
void CPrivateSendClientManager::GetJsonInfo(UniValue& obj) const
{
    ...
    obj.pushKV("max_denoms",    nPrivateSendDenoms);
