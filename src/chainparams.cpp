...
static CBlock CreateGenesisBlock(const CScript& genesisScriptSig, const CScript& genesisOutputScript, uint32_t nTime, uint32_t nNonce, uint32_t nBits, int32_t nVersion, Consensus::Params& consensus)
{
    ...
    dev.nVersion = 1;
    dev.setName("Xequium");
    dev.setShortName("XQUM");
    dev.nFlags = AssetMetadata::AssetFlags::ASSET_TRANSFERABLE | AssetMetadata::AssetFlags::ASSET_CONVERTABLE | AssetMetadata::AssetFlags::ASSET_STAKEABLE | AssetMetadata::AssetFlags::ASSET_LIMITED | AssetMetadata::AssetFlags::ASSET_RESTRICTED;
    dev.nExpiry =0;
    dev.nType =1;
    CAsset devasset = CAsset(dev);
    ...
    txNew.vout.push_back(CTxOut(devasset, 1*COIN, genesisOutputScript));

    txNew.nLockTime = 0;
    std::string txcomment = "www.coindesk.com/bitcoin-is-a-form-of-money-d-c-federal-court-rules-bloomberg";
    std::vector<uint8_t> vchdataIn(txcomment.begin(), txcomment.end());
...
public:
    CTestNetParams() {
        ...
        genesis = CreateGenesisBlock(1296688602, 414098458, 0x1d00ffff, 1, consensus);
        consensus.hashGenesisBlock = genesis.GetHash();
...
public:
    explicit SigNetParams(const ArgsManager& args) {
        ...
        genesis = CreateGenesisBlock(1598918400, 52613770, 0x1e0377ae, 1, consensus);
        consensus.hashGenesisBlock = genesis.GetHash();
...
public:
    explicit CRegTestParams(const ArgsManager& args) {
        ...
        genesis = CreateGenesisBlock(1296688602, 2, 0x207fffff, 1, consensus);
        consensus.hashGenesisBlock = genesis.GetHash();
