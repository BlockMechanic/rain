...
bool CFinalCommitment::Verify(const std::vector<CDeterministicMNCPtr>& members, bool checkSigs) const
{
    ...
    if (CountValidMembers() < params.minSize) {
        LogPrintf("invalid validMembers count. validMembersCount=%d\n", CountValidMembers());
        return false;
    }
    if (CountSigners() < params.minSize) {
        LogPrintf("invalid signers count. signersCount=%d\n", CountSigners());
        return false;
    }
    if (!quorumPublicKey.IsValid()) {
        LogPrintf("invalid quorumPublicKey\n");
        return false;
    }
    if (quorumVvecHash.IsNull()) {
        LogPrintf("invalid quorumVvecHash\n");
        return false;
    }
    if (!membersSig.IsValid()) {
        LogPrintf("invalid membersSig\n");
        return false;
    }
    if (!quorumSig.IsValid()) {
        LogPrintf("invalid vvecSig\n");
        return false;
    }

    for (size_t i = members.size(); i < params.size; i++) {
        if (validMembers[i]) {
            LogPrintf("invalid validMembers bitset. bit %d should not be set\n", i);
            return false;
        }
        if (signers[i]) {
            LogPrintf("invalid signers bitset. bit %d should not be set\n", i);
            return false;
        }
    }
    ...
    if (checkSigs) {
        ...
        if (!membersSig.VerifySecureAggregated(memberPubKeys, commitmentHash)) {
            LogPrintf("invalid aggregated members signature\n");
            return false;
        }

        if (!quorumSig.VerifyInsecure(quorumPublicKey, commitmentHash)) {
            LogPrintf("invalid quorum signature\n");
            return false;
        }
    }
...
bool CFinalCommitment::VerifySizes(const Consensus::LLMQParams& params) const
{
    if (signers.size() != params.size) {
        LogPrintf("invalid signers.size=%d\n", signers.size());
        return false;
    }
...
bool CheckLLMQCommitment(const CTransaction& tx, const CBlockIndex* pindexPrev, CValidationState& state)
{
    ...
    if (!GetTxPayload(tx, qcTx)) {
        return state.Invalid(ValidationInvalidReason::BADQC, false, REJECT_INVALID, "bad-qc-payload");
    }

    if (qcTx.nVersion == 0 || qcTx.nVersion > CFinalCommitmentTxPayload::CURRENT_VERSION) {
        return state.Invalid(ValidationInvalidReason::BADQC, false, REJECT_INVALID, "bad-qc-version");
    }

    if (qcTx.nHeight != pindexPrev->nHeight + 1) {
        return state.Invalid(ValidationInvalidReason::BADQC, false, REJECT_INVALID, "bad-qc-height");
    }

    if (!mapBlockIndex.count(qcTx.commitment.quorumHash)) {
        return state.Invalid(ValidationInvalidReason::BADQC, false, REJECT_INVALID, "bad-qc-quorum-hash");
    }
    ...
    if (!Params().GetConsensus().llmqs.count((Consensus::LLMQType)qcTx.commitment.llmqType)) {
        return state.Invalid(ValidationInvalidReason::BADQC, false, REJECT_INVALID, "bad-qc-type");
    }
    const auto& params = Params().GetConsensus().llmqs.at((Consensus::LLMQType)qcTx.commitment.llmqType);

    if (qcTx.commitment.IsNull()) {
        if (!qcTx.commitment.VerifyNull()) {
            return state.Invalid(ValidationInvalidReason::BADQC, false, REJECT_INVALID, "bad-qc-invalid-null");
        }
        return true;
    }
    ...
    if (!qcTx.commitment.Verify(members, false)) {
        return state.Invalid(ValidationInvalidReason::BADQC, false, REJECT_INVALID, "bad-qc-invalid");
    }
