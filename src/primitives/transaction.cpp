...
CMutableTransaction::CMutableTransaction() : nVersion(CTransaction::CURRENT_VERSION), nTime(time(0)), nLockTime(0) {}
...
CTransaction::CTransaction() : vin(), vout(), nVersion(CTransaction::CURRENT_VERSION), nTime(time(0)), nLockTime(0), hash{}, m_witness_hash{} {}
...
{
    std::string str;
    str += strprintf("CMutableTransaction(hash=%s, ver=%d, nTime=%d, vin.size=%u, vout.size=%u, nLockTime=%u)\n",
        GetHash().ToString(),
        nVersion,
        nTime,
        vin.size(),
