...
#ifndef RAIN_PRIMITIVES_TXWITNESS_H
#define RAIN_PRIMITIVES_TXWITNESS_H
...
public:
    ...
    {
        READWRITE(obj.vchIssuanceAmountRangeproof);
        READWRITE(obj.vchInflationKeysRangeproof);
...
public:
    ...
    SERIALIZE_METHODS(CTxOutWitness, obj)
    {
        READWRITE(obj.vchSurjectionproof);
        READWRITE(obj.vchRangeproof);
    }

    CTxOutWitness() {SetNull();}
...
public:
    ...
    SERIALIZE_METHODS(CTxWitness, obj)
    {
        ...
        for (size_t n = 0; n < obj.vtxoutwit.size(); n++) {
            READWRITE(obj.vtxoutwit[n]);
        }
        if (obj.IsNull()) {
...
#endif //RAIN_PRIMITIVES_TXWITNESS_H
