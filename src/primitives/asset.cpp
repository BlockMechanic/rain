...
std::string CAsset::ToString(bool mini) const
{
    ...
    str += strprintf("Details (ver=%d, transferable=%s, convertable=%s, limited=%s, restricted=%s, stakable=%s, type=%s, expiry=%d)\n", nVersion, isTransferable() ? "yes" : "no", isConvertable() ? "yes" : "no", isLimited() ? "yes" : "no", isRestricted() ? "yes" : "no", isStakeable() ? "yes" : "no", GetType(), GetExpiry());
