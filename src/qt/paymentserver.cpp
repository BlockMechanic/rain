...
void PaymentServer::ipcParseCommandLine(interfaces::Node& node, int argc, char* argv[])
{
    for (int i = 1; i < argc; i++)
    {
        ...
        // If the rain: URI contains a payment request, we are not able to detect the
        ...
        if (arg.startsWith(SUPERCOIN_IPC_PREFIX, Qt::CaseInsensitive)) // rain: URI
        {
            ...
            if (GUIUtil::parseRainURI(arg, &r) && !r.address.isEmpty())
            {
    ...
    optionsModel(nullptr)
{
    ...
    // on Mac: sent when you click rain: links
    ...
    if (startLocalServer)
    {
        ...
        if (!uriServer->listen(name)) {
            ...
            QMessageBox::critical(nullptr, tr("Payment request error"),
                tr("Cannot start rain: click-to-pay handler"));
        }
        else {
...
//
// OSX-specific way of handling rain: URIs and PaymentRequest mime types.
...
void PaymentServer::handleURIOrFile(const QString& s)
{
    ...
    if (s.startsWith("rain://", Qt::CaseInsensitive))
    {
    ...
    else if (s.startsWith(SUPERCOIN_IPC_PREFIX, Qt::CaseInsensitive)) // rain: URI
    {
        ...
        // normal URI
        {
            ...
            if (GUIUtil::parseRainURI(s, &recipient))
            {
                ...
                Q_EMIT message(tr("URI handling"),
                    tr("URI cannot be parsed! This can be caused by an invalid Rain address or malformed URI parameters."),
