        ...
        platformStyle(_platformStyle)
{
    ...
    connect(ui->ManageButton, SIGNAL(clicked()), this, SLOT(gotoManageAssetsPage()));
    connect(ui->TransferButton, SIGNAL(clicked()), this, SLOT(gotoAssetsPage()));
    connect(ui->RestrictedButton, SIGNAL(clicked()), this, SLOT(gotoRestrictedAssetsPage()));
}
...
void CreateAssetDialog::setModel(WalletModel *_model)
{
    ...
    if(_model && _model->getOptionsModel())
    {
        ...
        setBalance(balances);        
        connect(model, &WalletModel::balanceChanged, this, &CreateAssetDialog::setBalance);
        connect(model->getOptionsModel(), &OptionsModel::displayUnitChanged, this, &CreateAssetDialog::updateDisplayUnit);
        updateDisplayUnit();
...
//        connect(ui->optInRBF, SIGNAL(stateChanged(int)), this, SLOT(coinControlUpdateLabels()));
        ui->customFee->setSingleStep(GetRequiredFee(*model->getWallet(), 1000));
...
void CreateAssetDialog::setUpValues()
{
    ...
    QStringList list;
    list.append(tr("Main Asset") + " (" + RainUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), GetBurnAmount(AssetType::ROOT)) + ")");
    list.append(tr("Sub Asset") + " (" + RainUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), GetBurnAmount(AssetType::SUB)) + ")");
    list.append(tr("Unique Asset") + " (" + RainUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), GetBurnAmount(AssetType::UNIQUE)) + ")");
    list.append(tr("Messaging Channel Asset") + " (" + RainUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), GetBurnAmount(AssetType::MSGCHANNEL)) + ")");
    list.append(tr("Qualifier Asset") + " (" + RainUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), GetBurnAmount(AssetType::QUALIFIER)) + ")");
    list.append(tr("Sub Qualifier Asset") + " (" + RainUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), GetBurnAmount(AssetType::SUB_QUALIFIER)) + ")");
    list.append(tr("Restricted Asset") + " (" + RainUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), GetBurnAmount(AssetType::RESTRICTED)) + ")");
...
//    ui->frameAssetData->setStyleSheet(QString(".QFrame {background-color: %1; padding-top: 10px; padding-right: 5px; border: none;}").arg(platformStyle->WidgetBackGroundColor().name()));
    ...
    ui->sAssetNameLabel->setStyleSheet(STRING_LABEL_COLOR);
    ui->sAssetNameLabel->setFont(GUIUtil::getSubLabelFont());
...
void CreateAssetDialog::setBalance(const interfaces::WalletBalances& balances)
{
    ...
    {
        ui->labelBalance->setText(RainUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), balances.balance));
    }
}
...
void CreateAssetDialog::CheckFormState()
{
    ...
    std::string error;
    bool sAssetNameValid = IsTypeCheckNameValid(AssetTypeFromInt(type), name.toStdString(), error);
    ...
    if (!sAssetNameValid && name.size() != 0) {
    ...
    if (!(ui->addressText->text().isEmpty() || IsValidDestination(dest)) && sAssetNameValid) {
        ui->addressText->setStyleSheet(STYLE_INVALID);
        showMessage(tr("Warning: Invalid Rain address"));
        return;
    }
...
void CreateAssetDialog::onCreateAssetClicked()
{
    ...
    if (!CreateAssetTransaction(model->getWallet(), ctrl, asset, address.toStdString(), error, *tx, nFeeRequired, fRestrictedAssetCreation ? &verifierStripped : nullptr)) {
    ...
    QString amount = "<b>" + QString::fromStdString(ValueFromAmountString(GetBurnAmount(type), 8)) + " XQM";
    amount.append("</b>");
    ...
    if(nFeeRequired > 0)
    {
        ...
        questionString.append(RainUnits::formatHtmlWithUnit(model->getOptionsModel()->getDisplayUnit(), nFeeRequired));
    ...
    for (RainUnits::Unit u : RainUnits::availableUnits())
    {
        if(u != model->getOptionsModel()->getDisplayUnit())
            alternativeUnits.append(RainUnits::formatHtmlWithUnit(u, totalAmount));
    }
    questionString.append(tr("Total Amount %1")
                                  .arg(RainUnits::formatHtmlWithUnit(model->getOptionsModel()->getDisplayUnit(), totalAmount)));
    questionString.append(QString("<span style='font-size:10pt;font-weight:normal;'><br />(=%2)</span>")
                                  .arg(alternativeUnits.join(" " + tr("or") + "<br />")));

    SendConfirmationDialog confirmationDialog(tr("Confirm send assets"), questionString.arg(formatted.join("<br />")), "", "", SEND_CONFIRM_DELAY, this);
    ...
    std::string txid;
    if (!SendAssetTransaction(model->getWallet(), *tx, error, txid)) {
...
void CreateAssetDialog::onAssetTypeActivated(int index)
{
    ...
    // Set sAssetName when it is an original asset type
...
void CreateAssetDialog::updateSmartFeeLabel()
{
    ...
    CFeeRate feeRate = CFeeRate(GetMinimumFee(*model->getWallet(), 1000, coin_control, &feeCalc));

    ui->labelSmartFee->setText(RainUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), feeRate.GetFeePerK()) + "/kB");
...
void CreateAssetDialog::coinControlChangeEdited(const QString& text)
{
    if (model && model->getAddressTableModel())
    {
        ...
        {
            ui->labelCoinControlChangeLabel->setText(tr("Warning: Invalid Rain address"));
        }
        else // Valid address
        {
...
{
    ui->customFee->setValue(GetRequiredFee(*model->getWallet(), 1000));
}
...
void CreateAssetDialog::updateFeeMinimizedLabel()
{
    ...
    if (ui->radioSmartFee->isChecked())
        ...
        ui->labelFeeMinimized->setText(RainUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), ui->customFee->value()) + "/kB");
    }
}
...
void CreateAssetDialog::updateMinFeeLabel()
{
    if (model && model->getOptionsModel())
        ui->checkBoxMinimumFee->setText(tr("Pay only the required fee of %1").arg(
                RainUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), GetRequiredFee(*model->getWallet(), 1000)) + "/kB")
        );
}
...
void CreateAssetDialog::gotoManageAssetsPage()
{
    WalletView* pMainWindow = qobject_cast<WalletView*>(parent());
    if (pMainWindow)
        pMainWindow->gotoManageAssetsPage();
}

void CreateAssetDialog::gotoAssetsPage()
{
    WalletView* pMainWindow = qobject_cast<WalletView*>(parent());
    if (pMainWindow)
        pMainWindow->gotoAssetsPage();
}

void CreateAssetDialog::gotoRestrictedAssetsPage()
{
    WalletView* pMainWindow = qobject_cast<WalletView*>(parent());
    if (pMainWindow)
        pMainWindow->gotoRestrictedAssetsPage();
}
