    ...
    m_chain_state_size(chain_state_size)
{
    ...
    //ui->welcomeLabel->setText(ui->welcomeLabel->text().arg("Rain"));
    ui->storageLabel->setText(ui->storageLabel->text().arg("Rain"));
  ...
  //      ui->lblExplanation3->setVisible(false);
    ...
    ui->sizeWarningLabel->setText(
        tr("%1 will download and store a copy of the Rain block chain.").arg("Rain") + " " +
...
bool Intro::showIfNeeded(interfaces::Node& node, bool& did_show_intro, bool& prune)
{
    ...
    if(!fs::exists(GUIUtil::qstringToBoostPath(dataDir)) || gArgs.GetBoolArg("-choosedatadir", DEFAULT_CHOOSE_DATADIR) || settings.value("fReset", false).toBool() || gArgs.GetBoolArg("-resetguisettings", false))
    {
        ...
        intro.setWindowIcon(QIcon(":icons/rain"));
        did_show_intro = true;
   ...
   const QVector<QString> permissions({"android.permission.INTERNET",
     ...
     * override -datadir in the rain.conf file in the default data directory
     * (to be consistent with raind behavior)
     */
    if(dataDir != GUIUtil::getDefaultDataDirectory()) {
