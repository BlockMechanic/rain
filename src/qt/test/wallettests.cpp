...
uint256 SendCoins(CWallet& wallet, SendCoinsDialog& sendCoinsDialog, const CTxDestination& address, CAmount amount, bool rbf)
{
    ...
    entry->findChild<RainAmountField*>("payAmount")->setValue(amount);
...
//
//     QT_QPA_PLATFORM=xcb     src/qt/test/test_rain-qt  # Linux
//     QT_QPA_PLATFORM=windows src/qt/test/test_rain-qt  # Windows
//     QT_QPA_PLATFORM=cocoa   src/qt/test/test_rain-qt  # macOS
...
/*    TestChain100Setup test;
    ...
    }
    {
        ...
        reserver.reserve();
        //CWallet::ScanResult result = wallet->ScanForWalletTransactions(Params().GetConsensus().hashGenesisBlock, 0 , {} , reserver, true );
...

    {
        ...
        QString balanceComparison = RainUnits::formatWithUnit(unit, balance, false, RainUnits::SeparatorStyle::ALWAYS);
    ...
    // Amount input
    RainAmountField* amountInput = receiveCoinsDialog.findChild<RainAmountField*>("reqAmount");
...
void WalletTests::walletTests()
{
    ...
    if (QApplication::platformName() == "minimal") {
        ...
        QWARN("Skipping WalletTests on mac build with 'minimal' platform set due to Qt bugs. To run AppTests, invoke "
              "with 'QT_QPA_PLATFORM=cocoa test_rain-qt' on mac, or else use a linux or windows build.");
        return;
    }
#endif
    TestGUI(m_node);
