...
void HexGrid::initializeGrid()
{
    ...
    {
        Q_EMIT connTerrChanged(i, players_.at(i)->connectedTerritories());
    }

    // Using set function instead of assigning the value to playerTurn_ directly
    // because this will Q_EMIT the corresponding signal too
...
{
    Q_EMIT showAttackResult(attack, defense);
    ...
    if (attack > defense)
    {
        ...
        Q_EMIT connTerrChanged(players_.indexOf(attacker), attacker->connectedTerritories());
        Q_EMIT connTerrChanged(players_.indexOf(defender), defender->connectedTerritories());
    ...
    if (playersLeft_ == 1)
    {
        Q_EMIT victory(players_.indexOf(attacker), attacker->human());
        return;
    }
...
uint32_t HexGrid::nNonce() const
{
    return nNonce_;
}

void HexGrid::setnNonce(uint32_t nNonce)
{
    nNonce_ = nNonce;
}
