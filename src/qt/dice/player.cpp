#include <qt/dice/player.h>
#include <qt/dice/territory.h>
...
void Player::setPlayerNumber(int playerNumber)
{
    ...
    for (auto i = 1; i <= 6; i++)
    {
        auto path = QString(":/icons/Player%1_Dice%2").arg(playerNumber).arg(i);
...
void Player::setAddress(QString _address){
	address = _address;
}
