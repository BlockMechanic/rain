...
//    setFlags(Qt::FramelessWindowHint);
    ...
    m_RegisterPane = this->rootObject()->findChild<QObject*>("registerpane");
    m_frontPage = this->rootObject()->findChild<QObject*>("frontPage");

    if (m_RegisterPane) {
        connect(this->rootObject(), SIGNAL(requestAddr()), this, SLOT(requestAddress()));
    }
...
void RainGUI::setClientModel(ClientModel *clientModel)
{
...
void RainGUI::message(QString title, QString message, unsigned int style)
{
...
void RainGUI::requestAddress()
{
    QString address = m_walletModel->getAddressTableModel()->addRow(AddressTableModel::Receive, "", "", OutputType::LEGACY);

    QMetaObject::invokeMethod(m_RegisterPane, "setAddress",
                              Q_ARG(QVariant, address));
}

void RainGUI::createAsset(QString assetName, QString assetShortName , QString addressTo, QString inputamount, QString assetToUse, QString outputamount, bool transferable, bool convertable, bool restricted, bool limited)
{


}

void RainGUI::convertAsset(QString addressTo, QString inputamount, QString inputassetName, QString outputamount, QString outputassetName)
{


}

void RainGUI::sendRain(QString address, QString amountt, QString assetName)
{
    ...
    for(auto const& x : passetsCache->GetItemsMap()){
        if(x.second->second.asset.IsExplicit()){
            ...
            if(QString::fromStdString(x.second->second.asset.GetAsset().getName()) == assetName){
...
void RainGUI::setWalletModel(WalletModel *model)
{
    if(model && model->getOptionsModel())
    {
        ...
        stakesFilter->setSourceModel(model->getTransactionTableModel());
        ...
        this->rootContext()->setContextProperty("assettable_model", model->getAssetTableModel());
        this->rootContext()->setContextProperty("coincontrolmodel", model->getCoinControlModel());

        // Keep up to date with wallet
        interfaces::Wallet& wallet = model->wallet();
        interfaces::WalletBalances balances = wallet.getBalances();
        setBalance(balances);
        connect(model, &WalletModel::balanceChanged, this, &RainGUI::setBalance);

        connect(model->getOptionsModel(), &OptionsModel::displayUnitChanged, this, &RainGUI::updateDisplayUnit);

        // Receive and pass through messages from wallet model
        connect(model, &WalletModel::message, this, &RainGUI::message);

        // Handle changes in encryption status
        connect(model, &WalletModel::encryptionStatusChanged, this, &RainGUI::updateWalletStatus);

        // update SPV status
        connect(model, &WalletModel::spvEnabledStatusChanged, this, &RainGUI::updateWalletStatus);

        // Balloon pop-up for new transaction
        connect(model->getTransactionTableModel(), &TransactionTableModel::rowsInserted, this, &RainGUI::processNewTransaction);
...
void RainGUI::updateWalletStatus(int status)
{
    Q_UNUSED(status);
    this->rootContext()->setContextProperty("EncryptionStatus", QVariant(m_walletModel->getEncryptionStatus()));
    this->rootContext()->setContextProperty("HDStatus", QVariant(m_walletModel->privateKeysDisabled()));
    this->rootContext()->setContextProperty("SPVStatus", QVariant(m_walletModel->spvEnabled()));
...
void RainGUI::setBalance(const interfaces::WalletBalances& balances)
{
    ...
    QMetaObject::invokeMethod(m_frontPage, "updateBalance",
                              Q_ARG(QVariant, GUIUtil::formatMultiAssetAmount(tmp, unit, RainUnits::separatorAlways, "")),
                              Q_ARG(QVariant, GUIUtil::formatMultiAssetAmount(balances.immature_balance, unit, RainUnits::separatorAlways, "\n")),
                              Q_ARG(QVariant, GUIUtil::formatMultiAssetAmount(balances.unconfirmed_balance, unit, RainUnits::separatorAlways, "\n")),
                              Q_ARG(QVariant, GUIUtil::formatMultiAssetAmount(balances.stake, unit, RainUnits::separatorAlways, "\n")));
...
void RainGUI::setSendBalance(QString assetname)
{
    ...
    QMetaObject::invokeMethod(m_sendPane, "setsendbalance", Q_ARG(QVariant, GUIUtil::formatMultiAssetAmount(tmp, unit, RainUnits::separatorAlways, "")));
}
...
void RainGUI::encryptWallet(QString pass)
{
    ...
    bool res = m_walletModel->setWalletEncrypted(true, ppass);
...
void RainGUI::setSPVMode(bool state)
{
    if(!m_walletModel)
        return;

    m_walletModel->setSpvEnabled(state);
    updateWalletStatus();
}

bool RainGUI::getSPVMode()
{
    if(!m_walletModel)
        return false;

    return m_walletModel->spvEnabled();
    updateWalletStatus();
}

void RainGUI::updateSPVStatus()
{
    spvEnabledStatusChanged(m_walletModel->spvEnabled());
    updateWalletStatus();
}
...
void RainGUI::updateStakingIcon()
{
    ...
    uint64_t nWeight= m_walletModel->getStakeWeight();
    QString tooltip="";
    bool staking = false;
    if (m_walletModel->wallet().getLastCoinStakeSearchInterval() && nWeight)
    {
