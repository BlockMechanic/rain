...
void RainGUI::createActions()
{
    ...
    connect(sendMessagesAction, &QAction::triggered, this, &RainGUI::gotoSendMessagesPage);
    connect(messageAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(messageAction, &QAction::triggered, this, &RainGUI::gotoMessagesPage);
#endif
#endif // ENABLE_WALLET
    ...
    aboutAction = new QAction(platformStyle->TextColorIcon(":/icons/about"), tr("&About %1").arg(tr("Rain")), this);
    aboutAction->setStatusTip(tr("Show information about %1").arg(tr("Rain")));
    ...
    optionsAction->setStatusTip(tr("Modify configuration options for %1").arg(tr("Rain")));
    ...
    openAction->setStatusTip(tr("Open a rain: URI or payment request"));
    ...
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible Rain command-line options").arg(tr("Rain")));
...
void RainGUI::createToolBars()
{
    ...
    if(walletFrame)
    {
        Logo = new QPushButton(QIcon(":icons/rain"),"",nullptr);
...
void RainGUI::setClientModel(ClientModel *_clientModel)
{
...
*/
        ...
        if (optionsModel && trayIcon) {
            ...
            connect(optionsModel, &OptionsModel::hideTrayIconChanged, this, &RainGUI::setTrayIconVisible);
...
void RainGUI::createTrayIcon()
{
    ...
    if (QSystemTrayIcon::isSystemTrayAvailable()) {
        ...
        QString toolTip = tr("%1 client").arg("Rain") + " " + m_network_style->getTitleAddText();
...
void RainGUI::setAuxiliaryBlockRequestProgress(const QDateTime& blockDate, int requestesBlocks, int loadedBlocks, int processedBlocks)
{
...
void RainGUI::message(const QString& title, QString message, unsigned int style, bool* ret)
{
...
void RainGUI::setSPVStatus(int spvEnabled)
{
...
void RainGUI::setTrayIconVisible(bool fHideTrayIcon)
{
    if (trayIcon)
    {
...
#ifndef OS_ANDROID
void RainGUI::mousePressEvent(QMouseEvent *event)
{
    m_nMouseClick_X_Coordinate = event->x();
    m_nMouseClick_Y_Coordinate = event->y();
}

void RainGUI::mouseMoveEvent(QMouseEvent *event)
{
...
//    int max_width = 0;
    ...
    //{
    //    max_width = qMax(max_width, fm.width(RainUnits::longName(unit)));
    //}
