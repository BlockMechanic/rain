...
/** Rain unit definitions. Encapsulates parsing and formatting
   and serves as list model for drop-down selection boxes.
*/
class RainUnits: public QAbstractListModel
{
    Q_OBJECT

public:
    explicit RainUnits(QObject *parent);

    /** Rain units.
      @note Source: https://en.rain.it/wiki/Units . Please add only sensible ones
     */
    enum Unit
    {
        COIN,
    ...
    static QString simplestFormat(int unit, const CAmount& amount, int digits = 2, bool plussign = false, SeparatorStyle separators = separatorStandard);
