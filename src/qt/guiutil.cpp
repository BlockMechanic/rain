...
#include <qt/rainaddressvalidator.h>
...
QString formatRainURI(const SendCoinsRecipient &info)
{
    ...
    QString ret = QString("rain:%1").arg(bech_32 ? info.address.toUpper() : info.address);
    int paramCount = 0;
...
//  dataDir = mediaPath.toString()+"/.rain/";
//  dataDir = mediaPath.toString()+"/Android/obb/"+package.toString()+"/.rain"; 
...
bool openRainConf()
{
    ...
    /* Open rain.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Rain.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Rain (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / strprintf("Rain (%s).lnk", chain);
}
...
fs::path static GetAutostartFilePath()
{
    std::string chain = gArgs.GetChainName();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "rain.desktop";
    return GetAutostartDir() / strprintf("rain-%s.desktop", chain);
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        std::string chain = gArgs.GetChainName();
        // Write a rain.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=Rain\n";
        else
            optionFile << strprintf("Name=Rain (%s)\n", chain);
...
LSSharedFileListItemRef findStartupItemInList(CFArrayRef listSnapshot, LSSharedFileListRef list, CFURLRef findUrl)
{
    ...
    // loop through the list of startup items and try to find the rain app
...
bool GetStartOnSystemStartup()
{
    CFURLRef rainAppUrl = CFBundleCopyBundleURL(CFBundleGetMainBundle());
    if (rainAppUrl == nullptr) {
        return false;
    }

    LSSharedFileListRef loginItems = LSSharedFileListCreate(nullptr, kLSSharedFileListSessionLoginItems, nullptr);
    CFArrayRef listSnapshot = LSSharedFileListCopySnapshot(loginItems, nullptr);
    bool res = (findStartupItemInList(listSnapshot, loginItems, rainAppUrl) != nullptr);
    CFRelease(rainAppUrl);
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    LSSharedFileListRef loginItems = LSSharedFileListCreate(nullptr, kLSSharedFileListSessionLoginItems, nullptr);
    CFArrayRef listSnapshot = LSSharedFileListCopySnapshot(loginItems, nullptr);
    LSSharedFileListItemRef foundItem = findStartupItemInList(listSnapshot, loginItems, rainAppUrl);

    if(fAutoStart && !foundItem) {
        // add rain app to startup item list
        LSSharedFileListInsertItemURL(loginItems, kLSSharedFileListItemBeforeFirst, nullptr, nullptr, rainAppUrl, nullptr, nullptr);
    }
