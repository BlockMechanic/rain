...
static void SetupCliArgs()
{
    ...
    gArgs.AddArg("-rpcwallet=<walletname>", "Send RPC for non-default wallet on RPC server (needs to exactly match corresponding -wallet option passed to raind). This changes the RPC endpoint used, e.g. http://127.0.0.1:8332/wallet/<walletname>", ArgsManager::ALLOW_ANY, OptionsCategory::OPTIONS);
    ...
    if (response.status == 0) {
        ...
        if (failedToGetAuthCookie) {
            throw std::runtime_error(strprintf(
                ...
                GetConfigFile(gArgs.GetArg("-conf", SUPERCOIN_CONF_FILENAME)).string().c_str()));
        } else {
