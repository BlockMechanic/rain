...
bool CheckSpecialTx(const CTransactionRef& tx, const CBlockIndex* pindexPrev, CValidationState& state)
{
	...
	switch (tx->nType) {
        ...
        case TRANSACTION_COINBASE:
            return CheckCbTx(CTransaction(*tx), pindexPrev, state);
        case TRANSACTION_QUORUM_COMMITMENT:
            return llmq::CheckLLMQCommitment(CTransaction(*tx), pindexPrev, state);
	}

    return state.Invalid(ValidationInvalidReason::CONSENSUS, false, REJECT_INVALID, "bad-tx-type-check");
}

bool ProcessSpecialTx(const CTransactionRef& tx, const CBlockIndex* pindex, CValidationState& state)
{
    if (tx->nType == TRANSACTION_NORMAL) {
        return true;
    }
...
bool ProcessSpecialTxsInBlock(const CBlock& block, const CBlockIndex* pindex, CValidationState& state, bool fJustCheck, bool fCheckCbTxMerleRoots)
{
	...
	for (int i = 0; i < (int)block.vtx.size(); i++) {
		if (!CheckSpecialTx(block.vtx[i], pindex->pprev, state)) {
			// pass the state returned by the function above
			return false;
		}
		if (!ProcessSpecialTx(block.vtx[i], pindex, state)) {
			// pass the state returned by the function above
			return false;
		}
	}
