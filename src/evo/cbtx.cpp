...
bool CheckCbTx(const CTransaction& tx, const CBlockIndex* pindexPrev, CValidationState& state)
{
    if (tx.nType != TRANSACTION_COINBASE) {
        return state.Invalid(ValidationInvalidReason::BADCBTX, false, REJECT_INVALID, "bad-cbtx-type");
    }

    if (!tx.IsCoinBase()) {
        return state.Invalid(ValidationInvalidReason::BADCBTX, false, REJECT_INVALID, "bad-cbtx-invalid");
    }

    CCbTx cbTx;
    if (!GetTxPayload(tx, cbTx)) {
        return state.Invalid(ValidationInvalidReason::BADCBTX, false, REJECT_INVALID, "bad-cbtx-payload");
    }
    ...
    if (pindexPrev && pindexPrev->nHeight + 1 != cbTx.nHeight) {
        return state.Invalid(ValidationInvalidReason::BADCBTX, false, REJECT_INVALID, "bad-cbtx-height");
    }
...
bool CheckCbTxMerkleRoots(const CBlock& block, const CBlockIndex* pindex, CValidationState& state)
{
    ...
    if (pindex) {
        ...
        if (!CalcCbTxMerkleRootMNList(block, pindex->pprev, calculatedMerkleRoot, state)) {
            return state.Invalid(ValidationInvalidReason::BADCBTX, false, REJECT_INVALID, "bad-cbtx-mnmerkleroot");
        }
        ...
        if (cbTx.nVersion >= 2) {
            if (!CalcCbTxMerkleRootQuorums(block, pindex->pprev, calculatedMerkleRoot, state)) {
                return state.Invalid(ValidationInvalidReason::BADCBTX, false, REJECT_INVALID, "bad-cbtx-quorummerkleroot");
            }
