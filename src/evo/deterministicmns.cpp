...
bool CDeterministicMNManager::ProcessBlock(const CBlock& block, const CBlockIndex* pindex, CValidationState& _state, bool fJustCheck)
{
    ...
    try {
        ...
        return _state.Invalid(ValidationInvalidReason::BADPROTX, false, REJECT_INVALID, "failed-dmn-block");
...
bool CDeterministicMNManager::BuildNewListFromBlock(const CBlock& block, const CBlockIndex* pindexPrev, CValidationState& _state, CDeterministicMNList& mnListRet, bool debugLogs)
{
    ...
    for (int i = 1; i < (int)block.vtx.size(); i++) {
        ...
        if (tx.nType == TRANSACTION_PROVIDER_REGISTER) {
            ...
            Coin coin;
            if (!proTx.collateralOutpoint.hash.IsNull() && (!GetUTXOCoin(dmn->collateralOutpoint, coin) || coin.out.nValue.GetAmount() != Params().MasternodeCollateral() || coin.out.nAsset.GetAsset() != Params().GetConsensus().masternode_asset)) {
                // should actually never get to this point as CheckProRegTx should have handled this case.
                // We do this additional check nevertheless to be 100% sure
                return _state.Invalid(ValidationInvalidReason::BADPROTX, false, REJECT_INVALID, "bad-protx-collateral");
            }
            ...
            if (newList.HasUniqueProperty(proTx.addr)) {
                return _state.Invalid(ValidationInvalidReason::BADPROTX, false, REJECT_DUPLICATE, "bad-protx-dup-addr");
            }
            if (newList.HasUniqueProperty(proTx.keyIDOwner) || newList.HasUniqueProperty(proTx.pubKeyOperator)) {
                return _state.Invalid(ValidationInvalidReason::BADPROTX, false, REJECT_DUPLICATE, "bad-protx-dup-key");
            }
            ...
            if (!dmn) {
                return _state.Invalid(ValidationInvalidReason::BADPROTX, false, REJECT_INVALID, "bad-protx-hash");
            }
            auto newState = std::make_shared<CDeterministicMNState>(*dmn->pdmnState);
            ...
            if (!qc.commitment.IsNull()) {
                ...
                if (!quorumIndex || quorumIndex->GetBlockHash() != qc.commitment.quorumHash) {
                    ...
                    return _state.Invalid(ValidationInvalidReason::BADPROTX, false, REJECT_INVALID, "bad-qc-quorum-hash");
                }
...
bool CDeterministicMNManager::IsProTxWithCollateral(const CTransactionRef& tx, uint32_t n)
{
    ...
    if (tx->vout[n].nValue.GetAmount() != Params().MasternodeCollateral()) {
        return false;
    }
    if (tx->vout[n].nAsset.GetAsset() != Params().GetConsensus().masternode_asset) {
        return false;
    }
    return true;
}
