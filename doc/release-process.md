...
* Update translations (ping wumpus on IRC) see [translation_process.md](https://github.com/rain/rain/blob/master/doc/translation_process.md#synchronising-translations).
...
* Update `src/chainparams.cpp` chainTxData with statistics about the transaction count and rate. Use the output of the RPC `getchaintxstats`, see
  [this pull request](https://github.com/rain/rain/pull/12270) for an example. Reviewers can verify the results by running `getchaintxstats <window_block_count> <window_last_block_hash>` with the `window_block_count` and `window_last_block_hash` from your output.
* On both the master branch and the new release branch:
  - update `CLIENT_VERSION_MINOR` in [`configure.ac`](../configure.ac)
  - update `CLIENT_VERSION_MINOR`, `PACKAGE_VERSION`, and `PACKAGE_STRING` in [`build_msvc/rain_config.h`](/build_msvc/rain_config.h)
...
### Fetch and create inputs: (first time, or when dependency versions change)
    ...
    pushd ./gitian-builder
    mkdir -p inputs
    wget -P inputs https://raincore.org/cfields/osslsigncode-Backports-to-1.7.1.patch
...
rain-${VERSION}-i686-pc-linux-gnu.tar.gz
...
- Upload zips and installers, as well as `SHA256SUMS.asc` from last step, to the rain.org server
...
- Update other repositories and websites for new version
  ...
  - Update packaging repo
      ...
      - Notify BlueMatt so that he can start building [the PPAs](https://launchpad.net/~rain/+archive/ubuntu/rain)
  ...
  - This repo
      ...
      - Create a pinned meta-issue for testing the release candidate (see [this issue](https://github.com/rain/rain/issues/15555) for an example) and provide a link to it in the release announcements where useful
