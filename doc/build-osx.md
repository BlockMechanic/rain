...
This guide describes how to build raind, command-line utilities, and GUI on macOS
...
These tools must be installed in order to build Rain Core from source.
...
### 4. Clone Rain repository

`git` should already be installed by default on your system.
Now that all the required dependencies are installed, let's clone the Rain Core repository to a directory.
...
It is not necessary to build wallet functionality to run `raind` or  `rain-qt`.
To enable legacy wallets, you must install `berkeley-db@4`.
To enable [descriptor wallets](https://github.com/rain/rain/blob/master/doc/descriptors.md), `sqlite` is required.
...
Rain Core includes a GUI built with the cross-platform Qt Framework.
...
See the notes in [#7714](https://github.com/rain/rain/issues/7714).
...
You can deploy a `.dmg` containing the Rain Core application using `make deploy`.
...
There are many ways to configure Rain Core, here are a few common examples:
...
Run the following in your terminal to compile Rain Core:
...
Rain Core should now be available at `./src/raind`.
If you compiled support for the GUI, it should be available at `./src/qt/rain-qt`.

The first time you run `raind` or `rain-qt`, it will start downloading the blockchain.
This process could take many hours, or even days on slower than average systems.

By default, blockchain and wallet data files will be stored in:

``` bash
/Users/${USER}/Library/Application Support/Rain/
```

Before running, you may create an empty configuration file:

```shell
mkdir -p "/Users/${USER}/Library/Application Support/Rain"

touch "/Users/${USER}/Library/Application Support/Rain/rain.conf"

chmod 600 "/Users/${USER}/Library/Application Support/Rain/rain.conf"
```

You can monitor the download process by looking at the debug.log file:

```shell
tail -f $HOME/Library/Application\ Support/Rain/debug.log
```

## Other commands:

```shell
./src/raind -daemon      # Starts the rain daemon.
./src/rain-cli --help    # Outputs a list of command-line options.
./src/rain-cli help      # Outputs a list of RPC commands when the daemon is running.
./src/qt/rain-qt -server # Starts the rain-qt server mode, allows rain-cli control
```
