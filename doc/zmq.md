...
The Rain Core daemon can be configured to act as a trusted "border
router", implementing the rain wire protocol and relay, making
...
The ZeroMQ feature in Rain Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building raind:
...
For instance:
    ...
    $ raind -zmqpubhashtx=tcp://127.0.0.1:28332 \
               -zmqpubhashtx=tcp://192.168.1.2:28332 \
               -zmqpubrawtx=ipc:///tmp/raind.tx.raw \
...
These options can also be provided in rain.conf.
...
From the perspective of raind, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into raind directly. Furthermore, no information is
...
using. Raind appends an up-counting sequence number to each
