Rain Core version 0.18.0 is now available from:

  <https://raincore.org/bin/rain-core-0.18.0/>
...
To receive security and update notifications, please subscribe to:
  ...
  <https://raincore.org/en/list/announcements/join/>
...
`/Applications/Rain-Qt` (on Mac) or `raind`/`rain-qt` (on
Linux).
...
Rain Core is supported and extensively tested on operating systems
using the Linux kernel, macOS 10.10+, and Windows 7 and newer. It is not
recommended to use Rain Core on unsupported systems.

Rain Core should also work on most other Unix-like systems but is not
as frequently tested on them.

From 0.17.0 onwards, macOS <10.10 is no longer supported. 0.17.0 is
built using Qt 5.9.x, which doesn't support versions of macOS older than
10.10. Additionally, Rain Core does not yet change appearance when
...
`contrib/init/raind.service`, it has been changed to use
`/var/lib/raind` as the data directory instead of
`~rain/.rain`. When switching over to the new configuration file,
please make sure that the filesystem on which `/var/lib/raind` will
exist has enough space (check using `df -h /var/lib/raind`), and
...
- Four new options are available for configuring the maximum number of
  ...
  documentation](https://github.com/rain/rain/blob/master/doc/zmq.md#usage)
  for details.
...
- The `whitelistforcerelay` option is used to relay transactions from
  ...
  [contacting](https://raincore.org/en/contact/) the Rain Core
...
The systemd init file (`contrib/init/raind.service`) has been changed
to use `/var/lib/raind` as the data directory instead of
`~rain/.rain`. This change makes Rain Core more consistent with
...
`datadir` option in `/etc/rain/rain.conf` will have no effect.
...
- A new short [document](https://github.com/rain/rain/blob/master/doc/JSON-RPC-interface.md)
  about the JSON-RPC interface describes cases where the results of an
  RPC might contain inconsistencies between data sourced from different
  subsystems, such as wallet state and mempool state.  A note is added
  to the [REST interface documentation](https://github.com/rain/rain/blob/master/doc/REST-interface.md)
  indicating that the same rules apply.

- Further information is added to the [JSON-RPC
  documentation](https://github.com/rain/rain/blob/master/doc/JSON-RPC-interface.md)
  about how to secure this interface.

- A new [document](https://github.com/rain/rain/blob/master/doc/rain-conf.md)
  about the `rain.conf` file describes how to use it to configure
  Rain Core.

- A new document introduces Rain Core's BIP174 [Partially-Signed
  Rain Transactions
  (PSBT)](https://github.com/rain/rain/blob/master/doc/psbt.md)
...
- The [output script
  descriptor](https://github.com/rain/rain/blob/master/doc/descriptors.md)
...
- A new `--disable-bip70` option may be passed to `./configure` to
  prevent Rain-Qt from being built with support for the BIP70 payment
  protocol or from linking libssl.  As the payment protocol has exposed
  Rain Core to libssl vulnerabilities in the past, builders who don't
...
- The minimum required version of Qt (when building the GUI) has been
  ...
  system](https://github.com/rain/rain/blob/master/depends/README.md)
  provides 5.9.7)
...
- `deriveaddresses` returns one or more addresses corresponding to an
  [output descriptor](https://github.com/rain/rain/blob/master/doc/descriptors.md).
...
- `getaddressinfo` now returns an additional `solvable` boolean field
  when Rain Core knows enough about the address's scriptPubKey,
...
- `importprivkey` will preserve previously-set labels for addresses or
  ...
  wallet" in earlier releases of Rain Core, subsequently importing
...
- `importmulti` now supports importing of addresses from descriptors. A
  ...
  [here](https://github.com/rain/rain/blob/master/doc/descriptors.md).
...
- The 'account' API is removed after being deprecated in v0.17.  The
  ...
  v0.17](https://github.com/rain/rain/blob/master/doc/release-notes/release-notes-0.17.0.md#label-and-account-apis-for-wallet)
...
- `generate` is deprecated and will be fully removed in a subsequent
  ...
  in this version, restart raind with the `-deprecatedrpc=generate`
...
- A new `rain-wallet` tool is now distributed alongside Rain
...
This section describes planned changes to Rain Core that may affect
other Rain software and services.

- Since version 0.16.0, Rain Core’s built-in wallet has defaulted to
  ...
  widely-used software.  Starting with Rain Core 0.20 (expected about
  a year after 0.18), Rain Core will default to native segwit
  ...
  to bech32 addresses, and if the Rain Core project sees enough
  additional adoption, it will instead default to bech32 receiving
  addresses in Rain Core 0.19 (approximately November 2019).
  ...
  set the `addresstype=bech32` configuration option in any Rain Core
...
- This release allows peers that your node automatically disconnected
  ...
  which you don't have many other peers).  Previously, Rain Core
...
- A sub-project of Rain Core now provides Hardware Wallet Interaction
  (HWI) scripts that allow command-line users to use several popular
  hardware key management devices with Rain Core.  See their [project
  page](https://github.com/rain-core/HWI#readme) for details.

Security
--------

- This release changes the Random Number Generator (RNG) used from
  OpenSSL to Rain Core's own implementation, although entropy
  gathered by Rain Core is fed out to OpenSSL and then read back in
  when the program needs strong randomness. This moves Rain Core a
...
- On macOS, Rain Core now opts out of application CPU throttling
...
- #12490 Remove deprecated wallet rpc features from `rain_server` (jnewbery)
- #14138 Set `encrypted_batch` to nullptr after delete. Avoid double free in the case of NDEBUG (practicalswift)
- #14168 Remove `ENABLE_WALLET` from `librain_server.a` (jnewbery)
...
- #13926 [Tools] rain-wallet - a tool for creating and managing wallets offline (jnewbery)
...
- #14162 Also log and print messages or questions like raind (MarcoFalke)
...
- #14521 qt, docs: Fix `rain-qt -version` output formatting (hebasto)
...
- #11625 Add RainApplication & RPCConsole tests (ryanofsky)
...
- #14204 Move `interfaces/*` to `librain_server` (laanwj)
...
- #15154 Configure: rain-tx doesn't need libevent, so don't pull it in (luke-jr)
...
- #15388 Makefile.am: add rule for src/rain-wallet (Sjors)
...
- #11526 Visual Studio build configuration for Rain Core (sipsorcery)
- #15110 build\_msvc: Fix the build problem in `librain_server` (Mr-Leshiy)
...
- #13953 Fix deprecation in rain-util-test.py (isghe)
...
- #12775 Integration of property based testing into Rain Core (Christewart)
...
- #12246 Bugfix: Only run rain-tx tests when rain-tx is enabled (luke-jr)
...
- #14092 Dry run `bench_rain` as part `make check` to allow for quick identification of assertion/sanitizer failures in benchmarking code (practicalswift)
...
- #14985 Remove `thread_local` from `test_rain` (MarcoFalke)
...
- #14186 rain-cli: don't translate command line options (HashUnlimited)
- #14057 logging: Only log `using config file path_to_rain.conf` message on startup if conf file exists (leishman)
...
- #12255 Update rain.service to conform to init.md (dongcarl)
...
- #14497 Add doc/rain-conf.md (hebasto)
...
- #15354 Add missing `rain-wallet` tool manpages (MarcoFalke)
...
- #15765 correct rainconsensus_version in shared-libraries.md (fanquake)
- #15792 describe onlynet option in doc/tor.md (jonatack)
- #15802 mention creating application support rain folder on OSX (JimmyMow)
...
As well as everyone that helped translating on [Transifex](https://www.transifex.com/projects/p/rain/).
